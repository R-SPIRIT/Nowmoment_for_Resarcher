        ...
        if a[1] == 'PF':
            ...
            if b == '1':
                ...
                for j in range(len(c)):
                    ...
                    new=pd.concat([new1,new2,new3,new4,new5],axis=1)
    return new
#server
ID = str(sys.argv[1])
...
q1=datafillnull(str("rn1.csv"),open(str(columnindexPath)+str(ID)+str(".txt"),'rt'))
q1.to_csv(csvPath+str("pfload.csv")  , sep=',',encoding='euc-kr')
f = open(csvPath+str("pfload.csv"), 'rt')
fw = open(csvPath+str("pfload1.csv"), 'w')
...
info = ('{},{},{},{},{}'.format(2, pyname, ID, days ,test_read_lines))
