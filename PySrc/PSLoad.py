...
q3=datascaling(str("po1.csv"),open(str(columnindexPath)+str(columnindex),'rt'))
q3.to_csv(csvPath+str("psload.csv")  , sep=',',encoding='euc-kr')

fs = open(csvPath+str("psload.csv"), 'rt')
fws = open(csvPath+str("psload1.csv"), 'w')
liness = fs.readlines()
for lines in liness:
    jos=lines.split(",")[1:]    
    jos1=",".join(jos)
...
info = ('{},{},{},{},{}'.format(6, pyname, ID, days ,test_read_lines))
